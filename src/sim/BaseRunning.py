...
class BaseRunning:
    ...
    def advance_runners_on_out(self, event: str, batter: Batter):
        """
            Handles logic for advancing runners on out type hit.
            Groundballs result in lead runner being out and the batter replacing first base.
        """
    ...
    def _clear_base_and_move_up(self, batter, base, disp):
        ...
        self.bases[base] = 0
        self.advance_runners(batter, "walk") # Call it a walk to guarantee the runners move only 1 base up
    ...
    def display(self, runs_scored):
        """
            Provides a printout of the runner advancements on a play, and runs scored.
        """
        if (runs_scored > 0):
