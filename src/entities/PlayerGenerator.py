...
from .config import DEFAULT_ATTRIBUTE, DEFAULT_VAR, MAX_PITCHES, MAX_ITER, PLAYER_ATTRIBUTES, BATTER_NUMERICAL_ATTRIBUTES, PITCHER_NUMERICAL_ATTRIBUTES, PITCH_NUMERICAL_ATTRIBUTES, RIGHT_HANDED_PROB, DEFAULT_NUMBER
...
def randomize_player(player, numbers_taken):
    ...
    number = int(number_dist.calculate_random_percentile(0, 99, 0, mode="skew"))
    while (number in numbers_taken):
        number = int(number_dist.calculate_random_percentile(0, 99, 0, mode="skew"))  # Assigning random jersey number
...
def generate_batter_attributes(batter):
    for attr in BATTER_NUMERICAL_ATTRIBUTES:
        batter[attr] = attribute_dist.calculate_random_percentile()
    return batter

def generate_pitcher_attributes(pitcher):
    for attr in PITCHER_NUMERICAL_ATTRIBUTES:
        pitcher[attr] = attribute_dist.calculate_random_percentile()  
    ...
    for pitch in pitcher["pitches"]:
        for attr in PITCH_NUMERICAL_ATTRIBUTES:
            ...
            setattr(pitch, attr, pitch_dist.calculate_random_percentile()) # Assign a random value to the attribute
    return pitcher
